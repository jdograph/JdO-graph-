...
import { Canvas, useFrame, useThree } from 'https://esm.sh/@react-three/fiber@8'
...
function Whale({ position, timeOfDay }) {
  ...
  useEffect(() => {
    ...
    tailGeometry.rotateX(Math.PI); tailGeometry.rotateZ(Math.PI / 4)
    ...
    const breathMaterial = new THREE.MeshStandardMaterial({ color: '#a7f3d0', transparent: true, opacity: 0 })
  ...
  useFrame(() => {
    if (whaleRef.current) {
      const elapsedTime = clock.current.getElapsedTime()
      whaleRef.current.group.position.x += Math.sin(elapsedTime * 0.05) * 0.05
      whaleRef.current.group.position.z += Math.cos(elapsedTime * 0.05) * 0.05
...
function Dolphin({ position, timeOfDay }) {
  ...
  useEffect(() => {
    ...
    bodyGeometry.rotateX(Math.PI / 2)
    const bodyMaterial = new THREE.MeshStandardMaterial({ color: timeOfDay === 'night' ? '#6ee7b7' : '#34d399' })
    const body = new THREE.Mesh(bodyGeometry, bodyMaterial)
    group.add(body)
...
function Turtle({ position, timeOfDay }) {
  ...
  useEffect(() => {
    ...
    const shellMaterial = new THREE.MeshStandardMaterial({ color: timeOfDay === 'night' ? '#4b5563' : '#374151' })
    const shell = new THREE.Mesh(shellGeometry, shellMaterial)
    group.add(shell)
    
    const headGeometry = new THREE.BoxGeometry(0.8, 0.5, 0.6)
    const headMaterial = new THREE.MeshStandardMaterial({ color: timeOfDay === 'night' ? '#6b7280' : '#4b5563' })
    const head = new THREE.Mesh(headGeometry, headMaterial)
    ...
    for (let i = 0; i < 4; i++) {
      ...
      fin.position.set(Math.cos(angle) * 1.2, 0, Math.sin(angle) * 1.2)
...
function TropicalIsland({ position, scale = 1, rotation = 0, name, section, timeOfDay }) {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 8; i++) {
      ...
      const leavesMaterial = new THREE.MeshStandardMaterial({ color: timeOfDay === 'night' ? '#064e3b' : '#065f46' })
    ...
    scene.add(group)
    return () => scene.remove(group)
  }, [position, scale, rotation, name, section, timeOfDay, scene])
...
function Scene3D({ activeSection, setActiveSection, timeOfDay, isInBoat, setIsInBoat }) {
  ...
  const [dolphinPositions, setDolphinPositions] = useState([[50, -1, -30], [-30, -0.5, 20], [20, -1.5, -80]])
  const [turtlePositions, setTurtlePositions] = useState([[-20, -2, 10], [30, -1.5, -40], [-50, -2, -70]])
  ...
  return (
    <>
      ...
      {dolphinPositions.map((pos, i) => <Dolphin key={i} position={pos} timeOfDay={timeOfDay} />)}
      {turtlePositions.map((pos, i) => <Turtle key={i} position={pos} timeOfDay={timeOfDay} />)}
  ...
  return (
    <div className="app-container">
      <Canvas camera={{ position: [0, 20, 40], fov: 60, near: 0.1, far: 10000 }}>
        <Scene3D activeSection={activeSection} setActiveSection={setActiveSection} timeOfDay={timeOfDay} isInBoat={isInBoat} setIsInBoat={setIsInBoat} />
      </Canvas>
      <div className="overlay-ui">
              ...
              {['home', 'about', 'portfolio', 'contact'].map((section) => (
                <li key={section}>
                  <button className={`nav-btn-3d ${activeSection === section ? 'active' : ''}`} onClick={() => setActiveSection(section)}>
                    {section === 'home' && '🏝️ Accueil'}{section === 'about' && '🌴 À Propos'}{section === 'portfolio' && '🎨 Portfolio'}{section === 'contact' && '📩 Contact'}
                  </button>
                </li>
              ))}
            </ul>
        ...
        <div className="section-content">
          ...
          {/* Les autres volets conditionnels (about, portfolio, contact, boat-panel) restent identiques... */}
        </div>
      </div>
    </div>
  )
